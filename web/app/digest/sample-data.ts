/* X-Med — données d'exemple (mêmes que la maquette).
   ...
   Les articles portent leurs DEUX faces (fr/en), puces « Résumé IA » et texte lu
   compris : l'aperçu de démonstration se lit donc entièrement dans la langue du
   compte. Seul l'habillage éditorial (profil fictif, thèmes) dépend de la
   langue au niveau du digest — d'où `sampleDigestFor(locale)`. */
import type { Locale } from "@/lib/locale";
import type { Article, DigestData } from "./types";

const SAMPLE_ARTICLES: { lead: Article; articles: Article[] } = {
  ...
  lead: {
    ...
    en: {
      ...
      why: [
        "Updated meta-analysis covering more than 40,000 high-risk pregnancies.",
        "Relative risk reduction of about 33% when treatment starts before 16 weeks.",
        "No excess of maternal or neonatal bleeding reported.",
        "Supports risk stratification from the first antenatal visit.",
      ],
      spoken:
        "Article of the day. Aspirin confirms its place in the prevention of pre-eclampsia. An updated meta-analysis of more than forty thousand high-risk pregnancies shows that low-dose aspirin, started before sixteen weeks, cuts the risk by roughly one third, with no excess bleeding. In practice, these results support screening for risk from the very first antenatal visit.",
    },
  ...
  articles: [
    {
      ...
      en: {
        ...
        why: [
          "Prospective cohort comparing medical and surgical management.",
          "Endpoints: pain and fertility at 5 years.",
          "Modest differences between the two strategies.",
        ],
        spoken:
          "Deep endometriosis: should we operate? A five-year prospective cohort shows that pain and fertility differ little between medical management and surgery. The decision therefore remains individual, to be discussed with the patient.",
      },
    ...
    },
    {
      ...
      en: {
        ...
        why: [
          "Randomized trial on screening coverage.",
          "Higher participation among under-screened women.",
          "A practical option for reaching hard-to-reach populations.",
        ],
        spoken:
          "Cervical cancer screening: a randomized trial shows that offering HPV self-sampling markedly increases participation among women who are rarely or never screened. It is a practical option for improving screening coverage.",
      },
    ...
    },
    {
      ...
      en: {
        ...
        why: [
          "Randomized placebo-controlled trial, 52 weeks.",
          "CRP down 38%; IL-6 and fibrinogen also reduced.",
          "An effect only partly explained by weight loss.",
        ],
        spoken:
          "Semaglutide and inflammation. In the randomized STEP-INFLAM trial, semaglutide reduced C-reactive protein by thirty-eight percent at one year, an effect only partly explained by weight loss. Interleukin 6 and fibrinogen also fell.",
      },
    ...
    },
    {
      ...
      en: {
        ...
        why: [
          "Cell model: endometriotic stromal cells.",
          "Lower IL-6, IL-8 and TNF-α; NF-κB activation suppressed.",
          "Rationale for exploring GLP-1 in endometriosis.",
        ],
        spoken:
          "GLP-1 receptor agonists curb inflammation in endometriotic lesions. In stromal cells, treatment reduces secretion of interleukin 6, interleukin 8 and TNF alpha, with suppression of the NF kappa B pathway. This suggests a direct anti-inflammatory action, beyond the metabolic effect.",
      },
...
// Habillage du digest de démonstration, par langue. `date` est de toute façon
// réécrite par la page (jour courant) ; elle reste ici pour un rendu isolé.
const SAMPLE_SHELL: Record<Locale, Omit<DigestData, "lead" | "articles">> = {
  en: {
    date: "Thursday, 31 May 2026",
    generated: "06:00",
    method: "demo preview",
    doctor: { name: "Dr Lefèvre", specialty: "Medical gynaecology" },
    themes: ["Endometriosis", "Pelvic pain", "Reproduction", "Inflammation", "Screening"],
  },
  fr: {
...
export function sampleDigestFor(locale: Locale): DigestData {
  return { ...SAMPLE_SHELL[locale], ...SAMPLE_ARTICLES };
}
